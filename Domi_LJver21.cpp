...
int main() {
	...
	cvvfile << "t" << "," << "cvv_dt" << "," << "cvv_ave" << "," << "cvv_accum" << endl;
	...
	for (t = 0; t < tmax; t++) {
		...
		//cout << "v after first verlet= " << v[i][0] << endl;
		...
		//cvv_dt = (1/t_corr)*(SUM of all v_tzero * v_tplusdt);
										//the window index counter


		//cout << "v beforeb k = " << v[i][0] << endl;
		...
		//cout << "v recording loop is fine " << endl;
		...
		if (t > (t_corr * counter) + 1) {   //Cvv calculation will be skip for all t=0 and t=1 (the same applies fort =  10,11,20,21, and so on ) 
			...
			if (k > 0 && k <= 10) {
				...
				//cvvfile << t << "," << k << "," << cvv_ave[k] << "," << cvv_accum << endl;
