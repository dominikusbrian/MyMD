...
const int tmax_eq = 50000;						// maximum number of timestep or simulation duration
...
void Vel_Verlet_Langevin(double r[N][DIM], double v[N][DIM], double F[N][DIM], double& E_pot, double& E_kin, double& Temp) {
	...
	//F[i][0] = F[i][0] - (gamma * mass * v[i][0]) + (sqrt((randomforce/dt * (random[i] / dt))));
	//F[i][1] = F[i][1] - (gamma * mass * v[i][1]) + (sqrt((randomforce/dt * (random[i] / dt))));
	//F[i][2] = F[i][2] - (gamma * mass * v[i][2]) + (sqrt((randomforce/dt * (random[i] / dt))));
	cout << "The force is = " << F[i][0] << "," << F[i][1] << "," << F[i][2] << endl;
	}
	//updating velocity at half time step
	for (i = 0; i < N; i++) {
