		...
		for (j = i + 1; j < N; j++) {
			...
			for (k = 0; k < DIM; k++) {
				if (rij < rcut) {
					...
					rij6 = ((sigma2 * sigma2 * sigma2) / (rij * rij * rij));// sigma^6 / rij^6
					rij12 = ((sigma2 * sigma2 * sigma2 * sigma2 * sigma2 * sigma2) / (rij * rij * rij * rij * rij * rij));							// sigma^12 / rij^12
