...
void VV_Langevin_side(double r[N][DIM], double v[N][DIM], double F[N][DIM], double& E_pot, double& E_kin, double& Temp);
void VV_Langevin_end(double r[N][DIM], double v[N][DIM], double F[N][DIM], double& E_pot, double& E_kin, double& Temp);
void VV_Langevin_middle(double r[N][DIM], double v[N][DIM], double F[N][DIM], double& E_pot, double& E_kin, double& Temp);
...
int main() {
	...
	for (t = 0; t < tmax_prod; t++) {
		...
		//VV_Langevin_side(r, v, F, E_pot, E_kin, Temp);
		//VV_Langevin_end(r, v, F, E_pot, E_kin, Temp);
		VV_Langevin_middle(r, v, F, E_pot, E_kin, Temp);
		E_tot = E_kin + E_pot;
		Temp = ((E_kin / N) * 2 / (3 * kB));
			...
			if (k==N_frame-1) {
				for (x = 0; x < N_frame; x++) {
					...
					if (t >= tmax_prod-N_frame) {
						cvvfile << (index - tmax_prod - N_frame + 2000) << "," << (index - tmax_prod - N_frame + 2000) * dt << "," << cvv_accum[x] / cvvcount << endl;
					}
					index++;
...
void VV_Langevin_side(double r[N][DIM], double v[N][DIM], double F[N][DIM], double& E_pot, double& E_kin, double& Temp) {
	...
	// modify velocity by Langevin dynamics
	for (i = 0; i < N; i++) {
		random[i] = normal(generator);
		Ca[i] = random[i]; // gaussian distributed random number

		
		v[i][0] = (v[i][0] * C1 * mass) + (C2 * sqrt(1 / beta) * Ca[i]) / mass ;
		v[i][1] = (v[i][1] * C1 * mass) + (C2 * sqrt(1 / beta) * Ca[i]) / mass;
		v[i][2] = (v[i][2] * C1 * mass) + (C2 * sqrt(1 / beta) * Ca[i]) / mass;
...
void VV_Langevin_middle(double r[N][DIM], double v[N][DIM], double F[N][DIM], double& E_pot, double& E_kin, double& Temp) {
	...
	for (i = 0; i < N; i++) {
		...
		r[i][0] = r[i][0] + v_half[i][0] * half_dt;
		r[i][1] = r[i][1] + v_half[i][1] * half_dt;
		r[i][2] = r[i][2] + v_half[i][2] * half_dt;
