...
int main() {
	...
	for (t = 0; t < tmax; t++) {
		//cout << "NOW , time is zero , and the E_kin is = " <<E_kin<< endl;
		//for (i = 0; i < N; i++) {
			//cout << i << "," << r[i][0] << "," << r[i][1] << "," << r[i][2] << "," << v[i][0] << "," << v[i][1] << "," << v[i][2] << endl;
		//}
		//E_pot = 0;
		...
		//cout << "Doing verlet now !" << endl;
		Vel_Verlet(r, v, F, E_pot, E_kin);
		//cout << "verlet is done !" << endl;
		//E_pot += U_LJ;
...
void Config_atoms_lattice(double r[N][DIM], double v[N][DIM]) {
	...
	for (i = 0; i < N; i++) {
		for (k = 0; k < DIM; k++) {
			//v[i][k] = { 0 };
			/* generate random number between 0.5 to 1.5: */
			v[i][k] = ((double)rand() * (10.0 - 0.1)) / (double)RAND_MAX + 1.0;
...
void Force_Calc(double r[N][DIM], double F[N][DIM], double &E_pot) {
	...
	const double rcut = 2.5*sigma;			// Cut-off Radius
