...
void rdf(double r[N][DIM], double g_r[size_bin], double g_ave[size_bin], int& divider, int& t);
...
int main() {
	...
	cout << "Temperature = " << Init_Temp << endl;
	...
	for (t = 0; t < tmax; t++) {
		...
		if (t > 5) {
			// recall the value from t = 0 to t=5
			...
			for (i = 0; i < N; i++) {
				...
				//vy_tplus[k][i] = v[i][1];
				//vz_tplus[k][i] = v[i][2];
				//cout << "vx_tplus = " << vx_tplus[k][i] << endl;
				//cout << "vy_tplus = " << vy_tplus[k][i] << endl;
				//cout << "vz_tplus = " << vz_tplus[k][i] << endl;
	...
	myfile << "Sigma = " << sigma << endl;
	myfile << "Mass = " << mass << endl;
	myfile << ("Total simulation time %.2fs\n", (double)(clock() - tstart) / CLOCKS_PER_SEC);
	myfile.close();
	return 0;
...
// Initialize position
void Config_atoms_lattice(double r[N][DIM]) {
	int edge = 7;    // For rho = 0.5 >> edge = 2 gives fill up half of the box, 4 fill up a quarter of the box. 
	int i;
	double ix, iy, iz;
...
void Config_velocity(double v[N][DIM]) {
	...
	// the random number generator generate random number between 3-10
	for (i = 0; i < N; i++) {
		//cout << "v_mag is = " << v_mag << endl;
		v[i][0] = (v_mag * (((double)rand() * (10.0 - 2.1)) / (double)RAND_MAX + 1.0));
		v[i][1] = (v_mag * (((double)rand() * (10.0 - 2.1)) / (double)RAND_MAX + 1.0));
		v[i][2] = (v_mag * (((double)rand() * (10.0 - 2.1)) / (double)RAND_MAX + 1.0));
...
void rdf(double r[N][DIM], double g_r[size_bin], double g_ave[size_bin], int& divider , int& t) {
	...
	otherfile <<t<< "," << 0 << "," << delta_r << "," << 0 << "," << 0 << "," << 0 << "," << divider << "," << 0 << endl;
	for (n = 2; n < 22; n++) {
		count = 0;
		for (i = 0; i < N - 1; i++) {
			for (j = i + 1; j < N; j++) {
		...
		otherfile <<t<< "," << ((n - 1.0) * delta_r) / sigma << "," << n * delta_r << "," << count << "," << g_r[n] << "," << g_ave[n] << "," << divider << "," << g_ave[n] / divider << endl;
